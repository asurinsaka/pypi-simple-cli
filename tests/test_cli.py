...
def test_list_indexd(**kwargs):
    ...
    runner = CliRunner()
    result = runner.invoke(cli.main, ['list', 'indexd', ])
...
def test_latest_indexd(**kwargs):
    with (DATA_DIR / "fake_page.html").open() as f:
        kwargs['mock'].get(f"{PYPI_SIMPLE_ENDPOINT}indexd/", text=f.read())
    runner = CliRunner()
    result = runner.invoke(cli.main, ['latest', 'indexd', ])
    assert result.stdout == "3.0.2.dev6+feat.dev.2298.use.different.post.fix.for.version"